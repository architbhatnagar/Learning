package practice.Twilio;

/*
A company is performing an analysis on the computers at its main office.
 The computers are spaced along a single row.
  For each group of contiguous computers of a certain length, that is, for each segment,
   determine the minimum amount of disk space available on a computer.
    Return the maximum of these values as your answer.
     Example n = 4 the number of computers space i = (8, 2, 4, 6);
      x = 2 the segment length
       The free disk space of computers in each of the segments
        is [8 21 [2, 4) (4, 6).
        The minima of the three segments are
         [2, 2. 41.
         The maximum of these is 4 Function Description Complete the function segment in the editor below.
          segment has the following parameterisk int x: the segment length to analyze
...
class Solution {
    ...
    public static int[] segmentEfficient(int nums[], int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            maxDequeue.offer(i);
            maxDequeue.forEach(System.out::print);
            System.out.println();
...
public class DiskSpace {
    public static void main(String[] args) throws IOException {
        ...
        var x=Solution.segmentEfficient(arr, 3);
